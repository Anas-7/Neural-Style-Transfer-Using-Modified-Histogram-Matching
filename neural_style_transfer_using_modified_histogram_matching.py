...
# The code of GPU options is necessary if running on personal laptop. 
# Highly recommended to run this on Colab which provides 16GB GPU memory (Tesla T4), higher than typical laptops/PCs.
...
# Command for checking GPU in Colab. Same command can be run in the terminal of a computer without the exclamation mark
# !nvidia-smi -L

# Global Parameters
basewidth = baseheight = 256 # This parameter can be set as 512 in Google Colab
IMAGE_DIRECTORY = "C:\\Users\\mohdm\\Documents\\outputs\\" # The directory is "\content\" in Google Colab
...
class InputImagePreprocessing:
  def __init__(self, enable_normal_method = False, enable_image_blending = True, 
              enable_HSV = False, num_blocks = 4, laplacian_iterations = 5, 
              CLAHE_gridsize = 8, CLAHE_clipLimit = 8.0):
...
class NST:
  def __init__(self, input_image, content_image_name, content_image, 
              style_image_name, style_image, STYLE_LAYERS, CONTENT_LAYERS, 
              alpha = 1e-5, beta = 1e-2, optimizer_rate = 7, iterations = 200, 
              enable_normalization = False, enable_normal_method = False):
...
def main():
  ...
  content_image_name="homes2"
  style_image_name="highres4"
  content_image, style_image = LoadImages(content_image_name = content_image_name,style_image_name = style_image_name).load()
  ...
  CONTENT_LAYERS = [
    ("block4_conv1",1)]
  nst = NST(input_image, content_image_name, content_image, style_image_name, style_image, STYLE_LAYERS, CONTENT_LAYERS)
  nst.double_mode()
  # nst.single_mode(print_loss = True)
  nst.plotTotalLoss()
  nst.plotContentLoss()
  nst.plotStyleLoss()
